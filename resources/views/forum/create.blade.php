<x-public-layout
    :metaTitle="'New Thread in ' . $category->name . ' — ExamReady PH Community'"
...
>
    ...
    <div class="bg-slate-50 dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 py-3">
            <nav class="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
                ...
                <i class="fa-solid fa-chevron-right text-[8px]"></i>
                <span class="text-slate-700 dark:text-slate-300 font-medium">New Thread</span>
            </nav>
        </div>
    </div>
    ...
    <section class="py-10">
        <div class="max-w-3xl mx-auto px-4 sm:px-6">
            <h1 class="text-2xl font-extrabold text-slate-900 dark:text-white mb-6">Start a New Thread</h1>
            ...
            <form method="POST" action="{{ route('forum.store', $category) }}" class="card flat-card p-6 sm:p-8 space-y-5">
                ...
                <div class="flex items-center gap-3 pt-2">
                    <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold px-8 py-3 rounded-xl transition shadow-md flex items-center gap-2">
                        <i class="fa-solid fa-paper-plane"></i> Post Thread
                    </button>
                    <a href="{{ route('forum.category', $category) }}" class="text-sm font-medium text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 transition">Cancel</a>
                </div>
            </form>
        </div>
    </section>
